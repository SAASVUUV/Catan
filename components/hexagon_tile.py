...
class HexagonTile:
    ...
    def __init__(self, number, type, x, y, radius, x_tabletop, y_tabletop, tabletop):
        self.hexagon = Hexagon(radius, x, y, types_colors[type], terrain_type=type)
    ...
    def link(self):
        ...
        self.tile_left_top = self.tabletop[self.y_tabletop - 1][
            self.x_tabletop - 1] if self.x_tabletop > 0 and self.y_tabletop > 0 else None
        self.tile_right_top = self.tabletop[self.y_tabletop - 1][
            self.x_tabletop + 1] if self.x_tabletop < 8 and self.y_tabletop > 0 else None
        self.tile_left_bottom = self.tabletop[self.y_tabletop + 1][
            self.x_tabletop - 1] if self.x_tabletop > 0 and self.y_tabletop < 4 else None
        self.tile_right_bottom = self.tabletop[self.y_tabletop + 1][
            self.x_tabletop + 1] if self.x_tabletop < 8 and self.y_tabletop < 4 else None
    ...
    def extract_houses(self):
        return [self.house_bottom, self.house_top, self.house_bottom_left, self.house_bottom_right, self.house_top_left,
                self.house_top_right]

    def extract_roads(self):
        return [self.road_left, self.road_right, self.road_bottom_left, self.road_bottom_right, self.road_top_left,
                self.road_top_right]
    ...
    def render(self, surface):
        ...
        if self.number_sprite:
            surface.blit(self.number_sprite, self.number_rect.topleft)

        if self.hover.hovered:
            pygame.draw.polygon(surface, (255, 255, 255), self.hexagon.points, 2)
