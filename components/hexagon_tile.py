...
class HexagonTile:
    ...
    def __init__(self, number, type, x, y, radius, x_tabletop, y_tabletop, tabletop):
        ...
        # Criando referências para as peças adjascentes baseadas na matriz do tabuleiro
    ...
    def create_houses(self):
        ...
        if(self.tile_left):
            self.house_top_left = self.tile_left.house_top_right
            self.house_bottom_left = self.tile_left.house_bottom_right
        if(self.tile_left_top):
            self.house_top_left = self.tile_left_top.house_bottom
            self.house_top = self.tile_left_top.house_bottom_right
        if(self.tile_right_top):
            self.house_top = self.tile_right_top.house_bottom_left
            self.house_top_right = self.tile_right_top.house_bottom
        if(self.house_bottom_left): self.house_bottom_left = House(self.hexagon.vertex_bottom_left)
        if(self.house_top_left): self.house_top_left = House(self.hexagon.vertex_top_left)
        if(self.house_top): self.house_top = House(self.hexagon.vertex_top)
        if(self.house_top_right): self.house_top_right = House(self.hexagon.vertex_top_right)
        self.house_bottom_right = House(self.hexagon.vertex_bottom_right)
        self.house_bottom = House(self.hexagon.vertex_bottom)
    ...
    def create_roads(self):
        ...
        if(self.tile_left): self.road_left = self.tile_left.road_top_right
        ...
        self.road_right = Road(self.hexagon.edge_right)
        self.road_bottom_right = Road(self.hexagon.edge_bottom_right)
        self.road_bottom_left = Road(self.hexagon.edge_bottom_left)
